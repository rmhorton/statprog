...
inputPanel(
  sliderInput("m", label = "slope:",
              min = m_range[1], max = m_range[2], value = 0, step = diff(m_range)/100),
  sliderInput("b", label = "intercept:",
              min = b_range[1], max = b_range[2], value = 0, step = diff(b_range)/100),
...
renderPlot({
  ...
  #error <- y - prediction
  #SSE <- sum(error^2)
  plot(x,y, xlim=c(0, max(x)), ylim=c(0, max(y)), main="measurement space")
  if (input$show_fit)
      abline(b, m, lty=1, lwd=3, col="yellow")
  if (input$show_errors)
    for (i in seq_along(x))
      lines(c(x[i], x[i]), c(prediction[i], y[i]), col="orange")
  abline(input$b, input$m, lty=2, lwd=3, col="green")
  # m_range[1] + 0.1 * diff(m_range), b_range[1] + 0.9 * diff(b_range)
  ...
  if (input$show_fit)
    points(m, b, col="yellow", cex=3, pch=1)
