...
d <- function(FUN, X){
  ...
  c(NA, sapply(interiorPoints, numerical_derivative), NA)
}
```
...
plotFdF <- function(x, fun, dfun){
  ...
  lines(x, d(fun,x), lwd=3, col="yellow")  # numerical approximation
  lines(x, dfun(x), lty=2, col="red")  # candidate analytical derivative
}
```
...
```{r simple1}
f <- function(x) -x^3 - 20*x^2 + 20*x
df <- function(x) -3*x^2 - 40*x + 20
x <- seq(-12, 12, by=0.01)
plotFdF(x, f, df)
```
