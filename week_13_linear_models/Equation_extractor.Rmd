...
get_equation <- function(model, mode="math"){
  coeff <- format(coef(model), digits=4)
  # mode = "math" or "code"
  assignOp <- if (mode == "math") "=" else "<-"
  multOp <-  if (mode == "math") "" else "*"
  paste( "y", assignOp, coeff[1] , "+", paste( coeff[-1], names(coeff[-1]), sep=multOp, collapse=" + "))
}
