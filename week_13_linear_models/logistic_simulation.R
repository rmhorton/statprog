...
library(manipulate)
manipulate({
  df <- transform(df, score = 10^a * (carcinogens - mean(carcinogens)) + 10^b * (age - mean(age)))
  df$prob <- logistic(df$score)
  hist(df$prob, breaks=50)
}, a=slider(-9, 9, step=0.1, initial = 0), b=slider(-9, 9, step=0.1, initial = 0) )


df <- transform(df, score = 10^(-1.1) * (carcinogens - mean(carcinogens)) + 10^-3 * (age - mean(age)))
df$prob <- logistic(df$score)
hist(df$prob, breaks=50)

df$cancer <- factor(ifelse(df$prob > runif(N), "Yes", "No"))

fit <- glm(cancer ~ I(carcinogens - mean(carcinogens)) + I(age - mean(age)) - 1, data=df, family="binomial")
