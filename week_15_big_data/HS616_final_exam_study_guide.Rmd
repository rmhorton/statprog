...
*  PCA is a  technique to choose selective variables which matter in the data and compute the correlation in data only among those variables and EFA is a technique to extract linear relationship between all variables. 
...
__xxu26__
 The principal() function in `library("psych")`will perform a principal component analysis in R, starting with a matrix. The format is as the following. Which of the following decription is NOT correct regarding the parmaters?                
...
*  Selecting components that do not introduce any variance to the data.
...
__cpkaur__
 Which library out of the following is useful for manipulating datasets in R, focussing on "dataframe-like objects"?
...
*  Tabulates  a correlation matrix and plots a kernel density map
...
What do the boxes and their central horizontal lines represent in the plot?
...
__xxu26__
 Assume that `library(ggplot2)` has been loaded and database `pressure` is built-in. Which of the following lines of code are equivalent?
```
qplot(temperature, pressure, data=pressure, geom="line")                      # 1
ggplot(pressure, aes(x=temperature, y=pressure)) + geom_line()                # 2
qplot(temperature, pressure, data=pressure, geom=c("line", "point"))          # 3
ggplot(pressure, aes(x=temperature, y=pressure)) + geom_line() + geom_point() # 4
```
...
*  ui.r and server.r have to be called separately 
...
* The "dist" function using data from data-frame df2, creates a matrix called `distances` which represents the distances between the rows of df2
...
```
library("networkD3")
caffeineData <- data.frame(Source=c("N1", "C2", 
  "N3", "C3a", "N4", "C5", "N6", "C7", "C7a", "C7a", 
  "C3a", "N1", "N4", "N6", "C5", "C7"),
  Target=c("C2", "N3", "C3a", "N4", "C5", "N6", 
  "C7", "C7a", "N1", "C3a", "C7a", "CH3a", "CH3b", 
  "CH3c", "O1", "O2"))
...
__xxu26__
 In the partitioning approach, the most common method is the K-means cluster analysis. Which of the following statements describe steps in the K-means algorithm? 

* All of these statements describe steps in the K-means algorithm. 
...
```
How could you use it to estimate the outcome values for input values of 1, 2, and 3?

* `predict(mod, data.frame(train_x = c(1, 2, 3)))`
* `predict(mod, data.frame(train_x(1, 2, 3)))`
* `crossval(mod, data.frame(train_x = c(1, 2, 3)))`
* `coxph(mod, data.frame(train_x = c(1, 2, 3)))`
...
__cpkaur__
 Which function(s) is/are used to embed a shiny application within R Markdown?
...
* To approximate a solution where there are more equations than unknowns
*  To approximate a solution where there more unknowns than equations
*  To approximate a solution where there equations and unknowns are equal
*  To approximate a solution where the equations are multiples of the unknowns. 
...
__vchaudhuri__
Which plot in randomForest package predicts the change in response as one changes the variable?

* partial dependence plot 
* partial independence plot
* complete independence plot
* complete dependence plot 
...
__lakarbatti__
 What does dplyr's __group_by__ function do?
...
* Lurking variables can contribute to Simpson's Paradox.
* Combining unequal sized groups into a single data set can contribute to Simpson's Paradox.
...
__xxu26__
 A common approach for evaluating the statistical assumptions in a regression analysis is to apply the `plot()` function to the object returned by `lm()`. Doing so produces four graphs that are useful for evaluating the model fit. One of the graphes is "Residual versus Leverage graph" which identifies outliers, high-leverage points, and inluential observations. Which of the following is correct? 
...
b <- c(2,4,6,8)
df <- data.frame(a,b)
with (df, {a <- a + 10; print(a)})
```

* adds 10 to each value in column `a`, then prints the result.
