...
renderPlot({
    x <- seq(-10, 10, by=0.1)
    plot(x, dnorm(x, mean=as.numeric(input$m1), sd=as.numeric(input$s1)), 
                    type="l", lwd=2, col="blue", ylab="density")
    lines(x, dnorm(x, mean=as.numeric(input$m2), sd=as.numeric(input$s2)),
                    lwd=2, col="green", ylab="density")
